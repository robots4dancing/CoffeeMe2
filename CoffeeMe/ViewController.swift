...
class ViewController: UIViewController, MKMapViewDelegate {
        ...
        localSearch.start { (response, error) in
            if let err = error {
                ...
                for item in response!.mapItems {
                    self.addLocAndPinFor(placemarks: [item.placemark], title: "From Apple: \(item.name)")
                }
            }
        }
    }
    ...
    @IBAction func longPressed(gesture: UILongPressGestureRecognizer) {
        if gesture.state == .ended {
            ...
            let loc = Location(name: "Long Press \(coord.latitude), \(coord.longitude)", address: "N/A", lat: coord.latitude, lon: coord.longitude)
